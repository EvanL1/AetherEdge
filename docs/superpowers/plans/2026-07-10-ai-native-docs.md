...
- `tools/aether/src/mcp.rs` — the module doc comment (registration-time gating rationale), the `WRITE_TOOL_NAMES` const in the tests module (all 27 write tool names), and the `#[tool(description = ...)]` strings on `channels_write`, `channels_control`, `models_instances_measurement` (these descriptions already carry the danger warnings — quote them)
...
- `## The write surface` — a table of all 27 write tools (from `WRITE_TOOL_NAMES`) in three severity groups: **device-affecting** (`channels_write`, `channels_control`, `channels_adjust`, `models_instances_action` — these reach physical equipment: PCS setpoints, generator start/stop), **data-integrity** (`models_instances_measurement` — it spoofs live telemetry; anything downstream, including alarm rules and control rules, will act on the forged value), and **configuration** (channels/rules/alarm-rules CRUD, net MQTT/cert — no immediate device effect, but changed config governs future behavior).
