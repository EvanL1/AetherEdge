...
| 字段 | 必填 | 含义 |
|-------|----------|---------|
| `input_watermark` | yes | 装配考虑的最新实际观察结果 |
| `missing_ratio` | yes | 缺失的单元格除以所有所需的单元格， `[0, 1]` |
...
| `submitted_at` | yes | 应用程序创建请求的 UTC 时间 |
| `deadline` | yes | 处理器不得开始工作或返回接受的 UTC 时间result |
| `task` | yes | ID、正修订版本和与加载任务匹配的类型类型 |
| `binding` | yes | 已解析委托绑定 ID 和正值修订 |
| `processor_contract` | 是 | 由任务和处理器公布的契约 |
| `artifact` | 否 | 通用种类、系列和可选请求的版本；无路径或 URL |
| `frame` | yes | 有效的 `ProcessingFrame` |
| `options` | yes | 与 `kind` 匹配的类型化选项对象`task.kind` |
...
仅当显式启用数据处理时才会安装它们。 JWT 角色映射向查看者、工程师和管理员授予发现权限；流程执行仅限于工程师和管理员。面向处理器的 sidecar 路由仍然是单独的 `POST /v1/process` 边界。
