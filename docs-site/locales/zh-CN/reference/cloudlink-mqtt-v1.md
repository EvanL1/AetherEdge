...
本文档描述了实验性 CloudLink 候选者。公共 AetherContracts `v0.1.0-alpha.3` 版本具有唯一权限，AetherEdge 和 AetherCloud 提交相同的摘要固定消费者锁。当前的声明是完整的分发完整性、完整的夹具执行以及选择加入真正的双产品阿尔法工具，而不是生产互操作性一致性。 ADR-0017拥有边缘架构； ADR-0018 拥有公共发布消费。
...
| 交付删除 | 本地 `AsyncClient::publish` 接受后 | ADR 需要持久应用确认；仅内存基础 | 专用线轴仅在验证持久 ACK 后删除 |
...
有关完整主题集，请参阅 ADR-0017。所有候选发布和订阅均使用 QoS 1 和 `retain = false`。前缀是可配置的。前缀和网关段拒绝空值、`+`、`#`、NUL/控制字符以及类似路径遍历的空段。网关仅接收其自己的 `down/session`、`down/ack` 和 `down/replay` 主题。
