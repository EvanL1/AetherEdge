...
这是一个实验性的预发布互操作候选方案。公开的 AetherContracts `v0.1.0-alpha.3` 版本是唯一的契约权威，AetherCloud 和 AetherEdge 使用相同的摘要锁定使用方清单。当前只能声明已完整采用分发包并执行公开测试夹具，不能声明已具备生产协议一致性。共享消息代理的生产密钥生命周期仍未解决，alpha.3 持久 ACK 也尚未明确签名。

## 公开发布消费

离线使用方检查固定的带注释标签对象、提交、包大小和 SHA-256、发布清单 SHA-256，以及每个已采用目标的字节。它导入精确的 alpha.3 采用闭包，包括规范、配置档、门槛、故障分类、TCK 清单、结构定义、测试夹具和验证器闭包。`pending_imports` 为空。

本地 `contract-manifest.json`、`wire-profile.json`、身份验证结构定义、门槛文件和场景只记录集成历史或 AetherCloud 提案，不能覆盖 AetherContracts。分发完整性本身并不证明编解码器或行为一致性。完整的公开夹具和可选双端测试工具现在能够提供 alpha 证据，但身份验证和生产级耐久存储仍未达到门槛。默认检查离线运行；可选的持续集成检查只下载摘要精确锁定的版本。
...
- `apps/cloudlink` 通过现有应用程序用例进行路由，没有应用回执时不会生成 ACK。对于遥测，网桥可以使用 `PostgresTelemetryRepository` 提交的精确 ACK 投影；当前的消息代理测试组合根仍使用内存，只能提供一致性证据，不能提供生产级持久性。
- 当前的云遥测模型仅安全地映射单样本线批次，因为它索引记录，而 alpha.3 定位原子批次。在云冻结该内部映射之前，较大的批次会显式失败。还计划了数据丢失持久性； alpha 线束记录了应用事实，但不声明生产耐久性。
- 默认测试不需要消息代理、Docker、PostgreSQL 或云账户。可选的 alpha 工具会启动真实的 Mosquitto、AetherEdge 的 rumqttc 传输和文件持久队列，以及 AetherCloud 的 MQTT 入口和应用程序用例。
...
| 真实消息代理双端边缘/云工具 | 本地 Mosquitto 和 AWS IoT Core alpha 证据；身份验证门仍是提案 |
