...
响应状态为 `200 OK`，正文包含 `{ "items": [...], "nextCursor": ... }`；
其中 `nextCursor` 是字符串或 `null`。每个项目都包含不可变的审计主题、操作、
资源、结果、风险、确认、关联，以及可选的跟踪和证据摘要字段。
...
`/health` 和 `/api/v1/platform` 有意公开，不包含租户或基础设施实例。审核路由需要
`Authorization: Bearer <token>`。本地服务器通过
`AETHER_CLOUD_API_BEARER_TOKEN`、`AETHER_CLOUD_API_TENANT_ID`、
`AETHER_CLOUD_API_PROJECT_ID`、`AETHER_CLOUD_API_SUBJECT_ID` 和以逗号分隔的
`AETHER_CLOUD_API_PERMISSIONS` 解析已配置的主体；缺少任何必要配置时，系统都会拒绝
受保护的请求。令牌使用恒定时间比较。
