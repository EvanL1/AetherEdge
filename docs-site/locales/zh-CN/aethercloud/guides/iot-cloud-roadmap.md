...
**当前状态。** 传输中立域/应用程序/内存基础通过端口、协议选择、无损纪元/游标、防护、恢复、心跳和会话查询来实现凭证验证。还实现了实验性 JSON/MQTT 编解码器、代理适配器、应用程序桥、入口生命周期、Schema、测试夹具和真实代理工具。生产凭证生命周期、共享 AetherEdge 一致性、超时调度程序、PostgreSQL 会话/纪元状态、持久数据丢失事实、多实例所有权和背压仍在计划中，因此第 2 阶段尚未完成。已实现接受遥测 PostgreSQL inbox/cursor/receipt/ACK 事务和传递用例，但它们未完成会话持久性。 ADR-0015 修复了发布顺序：共享代理源身份验证、单接头线材配置、相同的测试夹具、双边缘/云代理线束、故障注入、耐崩溃的 ACK 持久性，然后是显式旧版切换。目标配置文件尚未实现的编解码器。
