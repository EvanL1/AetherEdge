...
SunSpec model expansion is not part of the AetherEdge kernel. A downstream Rust IO plugin may own the SunSpec model catalog, discover its Modbus register chain, and project the result into canonical point topology. Energy Packs may declare that deployment capability as a prerequisite, but they cannot embed or activate plugin runtime code. Standard AetherEdge configurations reject `sunspec_tcp` and `sunspec_rtu` when no downstream composition supplies them.
