...
//!
//! Every manager has one lazy coherent topology. Configuration-only no-op
//! refreshes do not require IO or a physically published SHM generation.
...
async fn create_test_db() -> (TempDir, SqlitePool) {
    ...
    common::schema::init_automation_schema(&pool).await.unwrap();
    common::schema::init_io_schema(&pool).await.unwrap();
    (tmp, pool)
}

async fn make_manager(root: &TempDir, pool: SqlitePool) -> InstanceManager {
    let product_loader = Arc::new(ProductLoader::new(pool.clone()));
    let topology = Arc::new(
        AutomationTopologyHandle::from_sqlite_lazy(
            root.path().join("live.shm"),
            root.path().join("health.shm"),
            &pool,
    ...
    InstanceManager::new(pool, product_loader, topology)
}
...
async fn insert_action_routing(pool: &SqlitePool) {
    ...
    sqlx::query(
        "INSERT OR IGNORE INTO adjustment_points \
         (channel_id, point_id, signal_name, min_value, max_value, step) \
         VALUES (1, 1, 'setpoint', 0.0, 100.0, 1.0)",
    )
    .execute(pool)
    .await
    .unwrap();
...
async fn test_refresh_routing_reports_authoritative_route_count() {
    ...
    let manager = make_manager(&tmp, pool).await;
...
///
/// This verifies that an unchanged configuration snapshot does not turn IO
/// liveness into a prerequisite for local instance/routing reconciliation.
#[tokio::test]
...
async fn test_refresh_routing_concurrent_calls() {
    let (tmp1, pool1) = create_test_db().await;
    let (tmp2, pool2) = create_test_db().await;

    let manager1 = Arc::new(make_manager(&tmp1, pool1).await);
    let manager2 = Arc::new(make_manager(&tmp2, pool2).await);
