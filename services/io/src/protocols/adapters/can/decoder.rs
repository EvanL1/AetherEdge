#![cfg_attr(
    test,
    allow(clippy::disallowed_methods, clippy::unnecessary_get_then_check)
)]
...
use super::config::{CanDataType, CanFrameCache, CanPoint};
...
pub struct PointManager {
    /// Read-plane points indexed by their complete SCADA identity.
    points: HashMap<(PointType, u32), CanPoint>,
}
...
impl PointManager {
    ...
    fn validate_point(point: &CanPoint) -> Result<()> {
        if !matches!(point.point_type, PointType::Telemetry | PointType::Signal) {
            return Err(GatewayError::Config(format!(
                "CAN point {} uses unsupported {} plane; CAN exposes read-only T/S mappings",
                point.point_id, point.point_type
            )));
        }
        Ok(())
    }

    /// Add one validated read-plane point to the manager.
    pub fn add_point(&mut self, point: CanPoint) -> Result<()> {
        Self::validate_point(&point)?;
        self.points
            .insert((point.point_type, point.point_id), point);
        Ok(())
    }

    /// Atomically validate and add a complete CAN point generation.
    ...
    /// Whether this compiled point generation contains no mappings.
    #[must_use]
    ...
    ///
    /// Returns typed read-plane points for successfully decoded mappings.
    /// Quality is always Good for decoded values (no bad quality from CAN decode).
    pub fn apply_mappings(&self, frame_cache: &CanFrameCache) -> Result<DataBatch> {
        let mut result = DataBatch::with_capacity(self.points.len());

        for ((point_type, point_id), point) in &self.points {
            if let Some(frame_data) = frame_cache.get(point.can_id) {
                match decode_point(point, frame_data) {
                    Ok(value) => {
                        result.add(DataPoint::new(*point_id, *point_type, value));
                    },
                    Err(_e) => {
...
mod tests {
    ...
    fn test_point_manager_apply_mappings() {
        ...
        manager
            .add_point(create_point(1, 0x351, 0, 0, 16, CanDataType::UInt16))
            .unwrap();
        manager
            .add_point(create_point(2, 0x356, 0, 0, 16, CanDataType::UInt16))
            .unwrap();
        ...
        assert!(result.iter().any(|point| {
            point.id == 1
                && point.point_type == PointType::Telemetry
                && point.value == Value::Integer(100)
        }));
        assert!(result.iter().any(|point| {
            point.id == 2
                && point.point_type == PointType::Telemetry
                && point.value == Value::Integer(200)
        }));
    }
    ...
    fn test_point_manager_missing_frame() {
        ...
        manager
            ...
            .unwrap();
        manager
            .add_point(create_point(2, 0x999, 0, 0, 16, CanDataType::UInt16))
            .unwrap(); // No data for this
        ...
        assert!(!result.iter().any(|point| point.id == 2));
    }

    #[test]
    fn point_manager_preserves_read_plane_and_rejects_write_planes() {
        let mut telemetry = create_point(1, 0x351, 0, 0, 16, CanDataType::UInt16);
        telemetry.point_type = PointType::Telemetry;
        let mut signal = create_point(2, 0x356, 0, 0, 16, CanDataType::UInt16);
        signal.point_type = PointType::Signal;
        let mut control = create_point(3, 0x351, 0, 0, 16, CanDataType::UInt16);
        control.point_type = PointType::Control;
        let mut adjustment = create_point(4, 0x351, 0, 0, 16, CanDataType::UInt16);
        adjustment.point_type = PointType::Adjustment;

        let mut manager = PointManager::new();
        manager.add_point(telemetry).unwrap();
        manager.add_point(signal).unwrap();
        assert!(manager.add_point(control).is_err());
        assert!(manager.add_point(adjustment).is_err());

        let mut cache = CanFrameCache::new();
        cache.update(0x351, &[0x64, 0x00, 0, 0, 0, 0, 0, 0]);
        cache.update(0x356, &[0x01, 0x00, 0, 0, 0, 0, 0, 0]);

        let batch = manager.apply_mappings(&cache).unwrap();
        assert_eq!(batch.len(), 2);
        assert!(batch.iter().any(|point| {
            point.id == 1
                && point.point_type == PointType::Telemetry
                && point.value == Value::Integer(100)
        }));
