...
if bash "$BUILDER" v0-contract amd64 \
    ...
    --manifest-only="$TEMP_DIR/extracted.json" >/dev/null 2>&1; then
    echo "installer accepted extracted Home Assistant support" >&2
    exit 1
fi

if bash "$BUILDER" v0-contract amd64 \
    --io-features=modbus,not-a-real-adapter \
